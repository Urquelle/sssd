...
KMP_glied_t *
kmp_glied(BSS_speicher_t *speicher, enum KMP_glied_art_e art, KMP_spanne_t spanne)
{
...
{
    KMP_glied_t *erg = kmp_glied(speicher, GLIED_GANZZAHL, spanne);

    erg->art      = GLIED_GANZZAHL;
    erg->gz.basis = basis;
    erg->gz.wert  = wert;
...
KMP_glied_t *
kmp_glied_bezeichner(BSS_speicher_t *speicher, KMP_spanne_t spanne, d32 wert)
{
    KMP_glied_t *erg = kmp_glied(speicher, GLIED_GANZZAHL, spanne);

    erg->art     = GLIED_BEZEICHNER;
    erg->dz.wert = wert;
...
KMP_glied_t *
kmp_glied_text(BSS_speicher_t *speicher, KMP_spanne_t spanne, d32 wert)
{
    KMP_glied_t *erg = kmp_glied(speicher, GLIED_GANZZAHL, spanne);
