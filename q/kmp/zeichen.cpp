...
Zeichen::Zeichen(const Zeichen& z)
    ...
    , _text(z.text())
    , _quelldatei(z.quelldatei())
    , _versatz(z.versatz())
    , _byte_länge(z.byte_länge())
{
}

Zeichen::Zeichen(std::string_view utf8_char, uint32_t versatz, uint8_t byte_länge,
                 std::string& quelldatei, std::string& text)
    : _utf8_char(utf8_char)
    , _text(text)
    , _quelldatei(quelldatei)
    , _byte_länge(byte_länge)
    , _versatz(versatz)
{
...
std::string
Zeichen::quelldatei() const
{
    return _quelldatei;
}

uint32_t
Zeichen::versatz() const
{
    return _versatz;
}
