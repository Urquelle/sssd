...
// ast_knoten {{{
Ast_Knoten::Ast_Knoten(Ast_Knoten::Art art, Spanne spanne)
    : _art(art)
    , _spanne(spanne)
    , _id(++global_id)
{
...
void
Ast_Knoten::ausgeben(int32_t tiefe, std::ostream& ausgabe)
{
    switch (_art)
    {
    #define X(N, W, B) case Ast_Knoten::N: ausgabe << std::string(tiefe*4, ' ') << std::string(B); break;
        Ast_Knoten_Art
    #undef X
    }
}
...
// }}}
// ausdruck {{{
Ausdruck::Ausdruck(Ast_Knoten::Art art, Spanne spanne)
    : Ast_Knoten(art, spanne)
{
}

Ausdruck_Ganzzahl::Ausdruck_Ganzzahl(Spanne spanne, int32_t wert)
    : Ausdruck(Ast_Knoten::AUSDRUCK_GANZZAHL, spanne)
    , _wert(wert)
{
}
...
Ausdruck_Dezimalzahl::Ausdruck_Dezimalzahl(Spanne spanne, float wert)
    : Ausdruck(Ast_Knoten::AUSDRUCK_DEZIMALZAHL, spanne)
    , _wert(wert)
{
}
...
Ausdruck_Bezeichner::Ausdruck_Bezeichner(Spanne spanne, std::string wert)
    : Ausdruck(Ast_Knoten::AUSDRUCK_BEZEICHNER, spanne)
    , _wert(wert)
{
}
...
Ausdruck_Binär::Ausdruck_Binär(Spanne spanne, Ausdruck_Binär::Binär_Op op, Ausdruck *links, Ausdruck *rechts)
    : Ausdruck(Ast_Knoten::AUSDRUCK_BINÄR, spanne)
    , _op(op)
    , _links(links)
    , _rechts(rechts)
{
...
Ausdruck_Unär::Ausdruck_Unär(Spanne spanne, Ausdruck_Unär::Unär_Op op, Ausdruck *ausdruck)
    : Ausdruck(Ast_Knoten::AUSDRUCK_UNÄR, spanne)
    , _op(op)
    , _ausdruck(ausdruck)
{
}
...
Ausdruck_Klammer::Ausdruck_Klammer(Spanne spanne, Ausdruck *ausdruck)
    : Ausdruck(Ast_Knoten::AUSDRUCK_KLAMMER, spanne)
    , _ausdruck(ausdruck)
{
}
...
Ausdruck_Index::Ausdruck_Index(Spanne spanne, Ausdruck *basis, Ausdruck *index)
    : Ausdruck(Ast_Knoten::AUSDRUCK_INDEX, spanne)
    , _basis(basis)
    , _index(index)
{
}
...
Ausdruck_Eigenschaft::Ausdruck_Eigenschaft(Spanne spanne, Ausdruck *basis, Ausdruck *eigenschaft)
    : Ausdruck(Ast_Knoten::AUSDRUCK_EIGENSCHAFT, spanne)
    , _basis(basis)
...
Ausdruck_Aufruf::Ausdruck_Aufruf(Spanne spanne, Ausdruck *basis, std::vector<Ausdruck *> argumente)
    : Ausdruck(Ast_Knoten::AUSDRUCK_AUFRUF, spanne)
    , _basis(basis)
...
Ausdruck_Brauche::Ausdruck_Brauche(Spanne spanne, std::string dateiname)
    : Ausdruck(Ast_Knoten::AUSDRUCK_BRAUCHE, spanne)
...
Ausdruck_Kompositum::Ausdruck_Kompositum(Spanne spanne, std::vector<Ausdruck_Kompositum::Eigenschaft *> eigenschaften,
    ...
    : Ausdruck(Ast_Knoten::AUSDRUCK_KOMPOSITUM, spanne)
...
// }}}
// spezifizierung {{{
Spezifizierung::Spezifizierung(Ast_Knoten::Art art, Spanne spanne)
    : Ast_Knoten(art, spanne)
    , _größe(0)
{
}
...
Spezifizierung_Bezeichner::Spezifizierung_Bezeichner(Spanne spanne, std::string name)
    : Spezifizierung(Ast_Knoten::SPEZIFIZIERUNG_BEZEICHNER, spanne)
    , _name(name)
{
}
...
Spezifizierung_Feld::Spezifizierung_Feld(Spanne spanne, Spezifizierung *basis, Ausdruck *index)
    : Spezifizierung(Ast_Knoten::SPEZIFIZIERUNG_FELD, spanne)
    , _basis(basis)
    , _index(index)
{
}
...
Spezifizierung_Zeiger::Spezifizierung_Zeiger(Spanne spanne, Spezifizierung *basis)
    : Spezifizierung(Ast_Knoten::SPEZIFIZIERUNG_ZEIGER, spanne)
    , _basis(basis)
{
}
...
Spezifizierung_Funktion::Spezifizierung_Funktion(Spanne spanne, std::vector<Spezifizierung *> parameter,
    ...
    : Spezifizierung(Ast_Knoten::SPEZIFIZIERUNG_FUNKTION, spanne)
...
// }}}
// deklaration {{{
Deklaration::Deklaration(Ast_Knoten::Art art, Spanne spanne, std::vector<std::string> namen)
    : Ast_Knoten(art, spanne)
    , _namen(namen)
...
Deklaration::Deklaration(Ast_Knoten::Art art, Spanne spanne, std::string name)
    : Ast_Knoten(art, spanne)
    , _namen({name})
{
}
...
Deklaration_Variable::Deklaration_Variable(Spanne spanne, std::vector<std::string> namen,
    ...
    : Deklaration(Ast_Knoten::DEKLARATION_VARIABLE, spanne, namen)
...
Deklaration_Funktion::Deklaration_Funktion(Spanne spanne, std::string name, std::vector<Deklaration *> parameter,
                                           Spezifizierung *rückgabe, Anweisung *rumpf)
    : Deklaration(Ast_Knoten::DEKLARATION_FUNKTION, spanne, name)
    , _parameter(parameter)
    , _rückgabe(rückgabe)
    , _rumpf(rumpf)
{
...
Deklaration_Funktion::ausgeben(int32_t tiefe, std::ostream &ausgabe)
{
    ...
    {
        ausgabe.put('\n');
        _rückgabe->ausgeben(tiefe+1, ausgabe);
    }
...
Deklaration_Schablone::Deklaration_Schablone(Spanne spanne, std::string name, std::vector<Deklaration *> eigenschaften)
    : Deklaration(Ast_Knoten::DEKLARATION_SCHABLONE, spanne, name)
    , _eigenschaften(eigenschaften)
{
}
...
Deklaration_Brauche::Deklaration_Brauche(Spanne spanne, std::string name, Ausdruck_Brauche *ausdruck)
    : Deklaration(Ast_Knoten::DEKLARATION_BRAUCHE, spanne, name)
    , _ausdruck(ausdruck)
{
}
...
// }}}
// anweisung {{{
Anweisung::Anweisung(Ast_Knoten::Art art, Spanne spanne)
    : Ast_Knoten(art, spanne)
{
}
...
Anweisung_Zuweisung::Anweisung_Zuweisung(Spanne spanne, Ausdruck *links, Glied *op, Ausdruck *rechts)
    : Anweisung(Ast_Knoten::ANWEISUNG_ZUWEISUNG, spanne)
    , _links(links)
    , _op(op)
    , _rechts(rechts)
{
...
Anweisung_Wenn::Anweisung_Wenn(Spanne spanne, Ausdruck *bedingung, Anweisung *rumpf, Anweisung_Wenn *sonst)
    : Anweisung(Ast_Knoten::ANWEISUNG_WENN, spanne)
...
Anweisung_Für::Anweisung_Für(Spanne spanne, Ausdruck *index, Ausdruck *bedingung, Anweisung *rumpf)
    : Anweisung(Ast_Knoten::ANWEISUNG_FÜR, spanne)
    , _index(index)
    , _bedingung(bedingung)
    , _rumpf(rumpf)
{
...
Anweisung_Block::Anweisung_Block(Spanne spanne, std::vector<Anweisung *> anweisungen)
    : Anweisung(Ast_Knoten::ANWEISUNG_BLOCK, spanne)
...
Anweisung_Deklaration::Anweisung_Deklaration(Spanne spanne, Deklaration *deklaration)
    : Anweisung(Ast_Knoten::ANWEISUNG_DEKLARATION, spanne)
...
Anweisung_Lade::Anweisung_Lade(Spanne spanne, std::string dateiname)
    : Anweisung(Ast_Knoten::ANWEISUNG_LADE, spanne)
    , _dateiname(dateiname)
{
}
...
Anweisung_Ausdruck::Anweisung_Ausdruck(Spanne spanne, Ausdruck *ausdruck)
    : Anweisung(Ast_Knoten::ANWEISUNG_AUSDRUCK, spanne)
    , _ausdruck(ausdruck)
{
}
...
Anweisung_Danach::Anweisung_Danach(Spanne spanne, Anweisung *anweisung)
    : Anweisung(Ast_Knoten::ANWEISUNG_DANACH, spanne)
    , _anweisung(anweisung)
{
}

Anweisung *
Anweisung_Danach::anweisung() const
{
    return _anweisung;
}

Anweisung_Weiche::Anweisung_Weiche(Spanne spanne, Ausdruck *ausdruck, std::vector<Muster *> muster)
    : Anweisung(Ast_Knoten::ANWEISUNG_WEICHE, spanne)
    , _ausdruck(ausdruck)
    , _muster(muster)
{
}
